...
| `Backspace` | reset to the spawn state (pose + position, gantry re-engaged); restart the deploy script afterwards for a clean cold start |

The viewer camera tracks the robot automatically (mouse still orbits and
zooms around it).
